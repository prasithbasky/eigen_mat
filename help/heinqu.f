C*****7**1*********2*********3*********4*********5*********6*********7**
      SUBROUTINE HEINQU ( CHEXIST )
C
C
CA*   Unterprogramm zum feststellen, ob die Steuerdatei fuer die
C     Belegung des Common-Blockes bereits existiert.  Name der Datei
C     ist HEINIT.DAT
C
CB*   Name = HEINQU Hilfsroutine Inquire HEINIT.DAT
C            ------
C
C
...
C
      INQUIRE ( FILE='HEINIT.DAT', EXIST=EX )
C
      IF ( EX ) THEN
         CHEXIST = 'J'
      ELSE
